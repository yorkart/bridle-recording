...
- Timeline：按顺序展示请求开始、响应开始、SSE 事件、WebSocket 帧和完成状态。
- SSE：展示完整事件列表、事件字段和未经派生改写的原始事件文本。
- WebSocket：展示帧序号、方向、时间、opcode、文本或 base64 payload。
- Metadata：展示请求、HTTP 响应、WebSocket、usage 和事件统计等元信息。
- Raw：展示解码后的请求 JSON、完整 buffered response 和原始文件位置。
...
当一个录制会话确认可用于回归测试时，点击“剪裁并导出”进入离线剪裁流程：

1. 勾选要保留的请求 / 响应对；顺序以原始 recording 为准。
2. 选择是否移除 tools、skills、apps、plugins、environment / permissions 等派生 prompt。
3. 输入需要统一替换的敏感原文；每行一个值。
4. 生成服务端预览，检查导出编号、user inputs 和处理后的请求 body。
5. 确认后导出测试集。
...
`raw/` 是从 recording 派生的测试集副本，不是新的事实来源。只有确认保留的请求 / 响应对会进入副本，并按选中顺序重新编号为连续序列。Header 默认按测试集安全策略处理；用户选择的敏感值和移除项只作用于导出副本。源 recording 不会被修改，`request_match.json`、`response_rewrite.json` 等 mock 派生文件也不会混入副本。
...
这意味着原始录制始终保留为事实来源，剪裁结果是可提交、可回放的派生产物。剪裁、脱敏或预览失败不会进入在线代理和在线录制路径。
